...
const char print_calc = ';';     // t.kind==print means that t is a print Token
const string prompt = "> ";
...
void calculate()
{
    while (cin) {
        cout << prompt;
        Token t = ts.get();
        while (t.kind == print_calc) t = ts.get();   // first discard all "prints"
...
try
{
    ...
    cout << "Enter '" << print_calc << "' to print the result; enter '" << quit << "' to quit.\n";
    calculate();
    return 0;
}
catch (exception& e) {
